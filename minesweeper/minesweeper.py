...
class MinesweeperAI():
	...
	def get_neighbors(self, cell):
		...
		for i in range(row - 1, row + 2):
			for j in range(col - 1, col + 2):
				if 0 <= i < self.height and 0 <= j < self.width and cell != (i, j):
					neighbors.add((i, j))
		return neighbors
	...
	def add_knowledge(self, cell, count):
		...
		while changed:
			...
			# Marking mines and safes
			safes = set()
			mines = set()

			# Compiling all safes and mines from knowledge base
			...
			# Only keeping sentences that are not empty
				...
				for sentence2 in self.knowledge:
					if sentence1 != sentence2:
						if sentence1.cells.issubset(sentence2.cells):
							...
							if new_sentence not in self.knowledge and new_sentence not in inferred_sentences and new_sentence.cells:
